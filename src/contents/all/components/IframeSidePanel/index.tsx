...
export class IframeSidePanelManager {
    ...
    // 显示侧边栏并发送总结页面命令
    public static showWithSummarize(): void {
        this.show();

        // 等待iframe加载完成后发送消息
        setTimeout(() => {
            const iframe = document.querySelector('.iframe-sidepanel-iframe') as HTMLIFrameElement;
            if (iframe && iframe.contentWindow) {
                iframe.contentWindow.postMessage({ action: 'summarize' }, '*');
            }
        }, 1000);
    }
}
